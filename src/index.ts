...
import { getCertificationRequirements } from './tools/certification-requirements.js';
import { getIncidentReportingRules } from './tools/incident-reporting.js';
import { getRegulatorInfo } from './tools/regulator-info.js';
import { checkApplicability } from './tools/check-applicability.js';
...
function openDatabase(): Db {
  // Dynamic import to support both @ansvar/mcp-sqlite and better-sqlite3
...
function handleToolCall(
  ...
  switch (name) {
    ...
    case 'get_provision':
      return getProvision(db, args as Parameters<typeof getProvision>[1]);
    case 'get_cybersecurity_requirements':
      return getCybersecurityRequirements(db, args as Parameters<typeof getCybersecurityRequirements>[1]);
    case 'get_certification_requirements':
      return getCertificationRequirements(db, args as Parameters<typeof getCertificationRequirements>[1]);
    case 'get_incident_reporting_rules':
      return getIncidentReportingRules(db, args as Parameters<typeof getIncidentReportingRules>[1]);
    case 'get_regulator_info':
      return getRegulatorInfo(db, args as Parameters<typeof getRegulatorInfo>[1]);
    case 'check_applicability':
      return checkApplicability(db, args as Parameters<typeof checkApplicability>[1]);
    case 'list_sources':
